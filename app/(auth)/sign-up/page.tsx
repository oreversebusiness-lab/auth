...
function SignUpInner() {
  ...
  const redirectTo = isDauth ? "/api/dauth?ref=" + encodeURIComponent(from) : ref || "/dashboard"
  ...
  return (
    <div className="w-full max-w-sm">
      <div className="mb-8 text-center">
        ...
        <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">Join the OreVerse ecosystem</p>
        {ref && !isDauth && (
          <p className="mt-2 text-xs text-indigo-600 dark:text-indigo-400">
            Will redirect to <span className="font-semibold">{ref}</span> after sign up
          </p>
        )}
        {isDauth && (
          <p className="mt-2 text-xs text-indigo-600 dark:text-indigo-400">
      ...
      <div className="flex flex-col gap-3">
        <button onClick={() => handleSocialSignUp("google")} disabled={socialLoading !== null} className="flex w-full items-center justify-center gap-3 rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm font-medium text-zinc-700 shadow-sm hover:bg-zinc-50 transition-colors dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700 disabled:opacity-50">
        ...
        <button onClick={() => handleSocialSignUp("github")} disabled={socialLoading !== null} className="flex w-full items-center justify-center gap-3 rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm font-medium text-zinc-700 shadow-sm hover:bg-zinc-50 transition-colors dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700 disabled:opacity-50">
      ...
      <form onSubmit={handleEmailSignUp} className="flex flex-col gap-4">
        <div>
          ...
          <input id="name" type="text" required value={name} onChange={(e) => setName(e.target.value)} className="mt-1 block w-full rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100" placeholder="John Doe" />
        </div>
        <div>
      ...
      <p className="mt-6 text-center text-sm text-zinc-600 dark:text-zinc-400">
        ...
        <Link href={`/sign-in${isDauth ? `?dauth=true&from=${encodeURIComponent(from)}` : ref ? `?ref=${encodeURIComponent(ref)}` : ""}`} className="font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400">Sign in</Link>
      </p>
    </div>
  )
}
